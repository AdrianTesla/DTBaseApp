...
namespace DT
{
	void BloomLayer::OnAttach()
	{
		...
		m_BloomProcessor.GetSettings().Radius = 8.0f;
	}
	...
	void BloomLayer::OnRender()
	{
		...
		color.g *= m_Emission;
		color.b *= m_Emission;
		Renderer2D::DrawRotatedQuad({ 0.0f, 0.0f }, m_SquareWidth, m_SquareHeight, glm::radians(30.0f), color);
		Renderer2D::EndScene();
	...
	void BloomLayer::OnUIRender()
	{
		...
		ImGui::DragFloat("Emission", &m_Emission, 0.05f, 0.0f, 100000.0f);
		ImGui::SliderFloat("Radius", &settings.Radius, 2.0f, 7.0f);
		...
		ImGui::SeparatorText("Square Properties");

		ImGui::SliderFloat("Width", &m_SquareWidth, 0.0f, 1.0f);
		ImGui::SliderFloat("Height", &m_SquareHeight, 0.0f, 1.0f);
